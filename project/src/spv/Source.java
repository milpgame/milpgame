...
String buildedLabelOrSymbol="";
String comment="";

boolean exitFromDo=false;
do {
number=fin.read(buffer);
if (number!=-1)
{
if( buffer[0]== '$')
{
                       ...
                       if( buffer[0]== ')')
                        {
                         ...
                         char caracter=(char)buffer[0];
                         comment=comment+'$'+caracter ;
                         positionInTheFile=positionInTheFile+2;
                        }
                        }
...
} else if ((buffer[0]==32 || buffer[0]==13 ||
  buffer[0]==10 )&& buildLabelOrSymbol==false)
            {
              positionInTheFile=positionInTheFile+1;
              if (buffer[0]==10)
                               {
...
else if (buffer[0]!='$'&& buffer[0]!=32 && buffer[0]!=13
                               &&  buffer[0]!=10 )
...
char caracter=(char)buffer[0];
buildedLabelOrSymbol=buildedLabelOrSymbol+caracter ;
positionInTheFile=positionInTheFile+1;
}

//closing the construction of a label or a symbol
else if ((buffer[0]==32 || buffer[0]==13 || buffer[0]==10  )
                     && buildLabelOrSymbol==true)
      {
        positionInTheFile=positionInTheFile+1;
        typeSymbol=Source.T_LABEL_OR_SYMBOL;
        codeSymbol=Source.C_ANYOTHER_SYMBOL;
        textSymbol=buildedLabelOrSymbol;
...
else if (buffer[0]=='$'&& buildLabelOrSymbol==true)
{
positionInTheFile=positionInTheFile+1;
...
catch(IOException e)
{
   ...
   JOptionPane.showMessageDialog
                    (null,
                     "Error reading from file[ o.s.]! "+e.getMessage(),
                     "Error!",
                     JOptionPane.INFORMATION_MESSAGE
                     );
//System.exit(-1);
